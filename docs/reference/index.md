# Package index

## Core SDG Analytics

- [`fetch_sdg_country_data()`](https://imanojkumar.github.io/MineSDG/reference/fetch_sdg_country_data.md)
  : Fetch Country-Level SDG Data from UN SDG API
- [`list_sdg_indicators()`](https://imanojkumar.github.io/MineSDG/reference/list_sdg_indicators.md)
  : List SDG Indicators and Descriptions
- [`validate_sdg_indicator()`](https://imanojkumar.github.io/MineSDG/reference/validate_sdg_indicator.md)
  : Validate SDG Indicator Code
- [`get_sdg_metadata()`](https://imanojkumar.github.io/MineSDG/reference/get_sdg_metadata.md)
  : Internal: Get SDG Metadata (Cached)
- [`analyze_sdg_trend()`](https://imanojkumar.github.io/MineSDG/reference/analyze_sdg_trend.md)
  : Analyze SDG Indicator Trend
- [`compute_sdg_stability()`](https://imanojkumar.github.io/MineSDG/reference/compute_sdg_stability.md)
  : Compute SDG Stability Metrics
- [`benchmark_sdg_performance()`](https://imanojkumar.github.io/MineSDG/reference/benchmark_sdg_performance.md)
  : Benchmark SDG Performance
- [`analyze_sdg_convergence()`](https://imanojkumar.github.io/MineSDG/reference/analyze_sdg_convergence.md)
  : Analyze SDG Convergence
- [`generate_sdg_executive_summary()`](https://imanojkumar.github.io/MineSDG/reference/generate_sdg_executive_summary.md)
  : Generate Executive Summary for SDG Performance
- [`summarize_sdg_data()`](https://imanojkumar.github.io/MineSDG/reference/summarize_sdg_data.md)
  : Summarize SDG Indicator Data

## Visualization Layer

- [`plot_sdg_trend()`](https://imanojkumar.github.io/MineSDG/reference/plot_sdg_trend.md)
  : Plot SDG Indicator Trend
- [`plot_sdg_benchmark()`](https://imanojkumar.github.io/MineSDG/reference/plot_sdg_benchmark.md)
  : Plot SDG Benchmark Comparison
- [`plot_sdg_volatility()`](https://imanojkumar.github.io/MineSDG/reference/plot_sdg_volatility.md)
  : Plot SDG Volatility
- [`plot_sdg_convergence()`](https://imanojkumar.github.io/MineSDG/reference/plot_sdg_convergence.md)
  : Plot SDG Convergence

## Mining-Sector Intelligence

- [`map_sdg_to_mining_domain()`](https://imanojkumar.github.io/MineSDG/reference/map_sdg_to_mining_domain.md)
  : Map SDG Indicator to Mining-Sector Domain
- [`calculate_land_restoration()`](https://imanojkumar.github.io/MineSDG/reference/calculate_land_restoration.md)
  : Calculate Land Restoration Rehabilitation Rate
- [`calculate_water_efficiency()`](https://imanojkumar.github.io/MineSDG/reference/calculate_water_efficiency.md)
  : Calculate Water Use Efficiency (SDG 6.4)
- [`generate_mining_risk_profile()`](https://imanojkumar.github.io/MineSDG/reference/generate_mining_risk_profile.md)
  : Generate Mining SDG Risk Profile
